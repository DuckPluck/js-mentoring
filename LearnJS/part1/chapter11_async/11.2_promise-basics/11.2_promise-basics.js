...
// Promise это конструктор, который создает объекты, на которые можно подписаться для получения результатов, когда они станут доступны.
...
// `.finally(() => handler)` - выполнит handler в любом случае, когда промис поменяет состояние. Не принимает аргументов ошибки или успеха
...
// Промисы это микрозадачи, то есть они выполняются раньше макрозадач (setTimeout, setInterval, XHR) и одновременно, при синхронном вызове
